...
object Http4sTestClient {
  ...
  def testClient(defaults: BQClientDefaults): Resource[IO, QueryClient[IO]] =
    retryResource(attempts = 3, delay = 2.seconds) {
      for {
  ...
  private def retryResource[A](attempts: Int, delay: FiniteDuration)(r: Resource[IO, A]): Resource[IO, A] =
    r.handleErrorWith { error =>
      if (attempts <= 1) Resource.raiseError[IO, A, Throwable](error)
      else Resource.eval(IO.sleep(delay)) >> retryResource(attempts - 1, delay)(r)
    }
}
