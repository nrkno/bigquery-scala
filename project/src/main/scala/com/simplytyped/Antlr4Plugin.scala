...
object Antlr4Plugin extends AutoPlugin {
  ...
  private val antlr4BuildDependency =
    settingKey[ModuleID]("Build dependency required for parsing grammars, scoped to plugin")
  ...
  def antlr4GeneratorTask: Def.Initialize[Task[Seq[File]]] = Def.task {
    ...
    val cachedCompile =
      FileFunction.cached(streams.value.cacheDirectory / "antlr4", FilesInfo.lastModified, FilesInfo.exists) {
        in: Set[File] =>
          runAntlr(
            srcFiles = in,
  ...
  override def projectSettings = inConfig(Antlr4)(
    Seq(
      ...
      javaSource := (Compile / target).value / "antlr4",
