...
abstract class BQSmokeTest extends CatsEffectSuite {
  ...
  protected def bqCheckTest[A](testName: String)(query: BQQuery[A])(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, query.sql, CheckType.Type(query.bqRead.bqType), assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqChecksTest[A](testName: String)(queries: List[BQQuery[A]])(implicit loc: Location): Unit =
    test(s"bqChecks: $testName".tag(TestTags.Generated)) {
      val combinedSql = queries.map(_.sql).mkFragment(bqfr"SELECT * FROM (", bqfr") UNION ALL (", bqfr")")
      bqCheckFragment(testName, combinedSql, CheckType.Type(queries.head.bqRead.bqType), assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqCheckFragmentNoSchemaTest(testName: String)(frag: BQSqlFrag)(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, frag, CheckType.Untyped, assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqCheckExample(testName: String)(frag: BQSqlFrag)(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, frag, CheckType.Untyped, assertStableTables, BQSmokeTest.exampleQueries)(bqClient())
    }

  // note that we just compare the generated sql for legacy sql. neither dry runs nor schemas work
  protected def bqCheckLegacyFragmentTest[A](testName: String)(frag: BQSqlFrag)(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      BQSmokeTest.queries.writeAndCompare(BQSmokeTest.queries.testFileForName(s"$testName.sql"), frag.asStringWithUDFs)
    }

  protected def bqCheckFragmentTest(testName: String)(tuple: (BQSqlFrag, BQSchema))(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, tuple._1, CheckType.Schema(tuple._2), assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqCheckFill(testName: String)(fill: BQFill)(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, fill.query, CheckType.Schema(fill.tableDef.schema), assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqCheckViewTest(testName: String, view: BQTableDef.ViewLike[Any])(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, view.query, CheckType.Schema(view.schema), assertStableTables, BQSmokeTest.queries)(bqClient())
    }

  protected def bqCheckFragmentTestFailing(testName: String, errorFragment: String)(frag: BQSqlFrag)(implicit loc: Location): Unit =
    test(s"bqCheck: $testName".tag(TestTags.Generated)) {
      bqCheckFragment(testName, frag, CheckType.Failing, assertStableTables, BQSmokeTest.queries)(bqClient()).attempt
        .flatMap {
          case Left(e: BigQueryException) if e.getMessage != null => IO(assert(e.getError.getMessage.contains(errorFragment)))
          case Left(other)                                        => IO.raiseError(other)
          case Right(_)                                           => IO(fail(s"Expected BQ to report failure with the substring '$errorFragment'"))
        }
    }
}
...
private object BQSmokeTest {
  ...
  lazy val (bqClient, cleanup) = BigQueryTestClient.testClient.allocated.unsafeRunSync()
  ...
  def bqCheckFragment(
    ...
    val compareAsIs = IO(target.writeAndCompare(target.testFileForName(s"$testName.sql"), frag.asStringWithUDFs))

    dependenciesAsStaticData(frag, assertStable) match {
      case Right(staticFrag) =>
        val compareStatic = IO(staticQueries.writeAndCompare(staticQueries.testFileForName(s"$testName.sql"), staticFrag.asStringWithUDFs))
          ...
          .flatMap {
            ...
            case None =>
              ...
              val run = bqClient
                ...
                .guaranteeCase {
                  case Outcome.Errored(_) if checkType != CheckType.Failing => IO(println(s"failed query: ${staticFrag.asStringWithUDFs}"))
                  case _                                                    => IO.unit
                }
      ...
      case Left(notStaticBecause) =>
        ...
        val runCheck = bqClient
          ...
          .guaranteeCase {
            case Outcome.Errored(_) if checkType != CheckType.Failing => IO(println(s"failed query: ${frag.asStringWithUDFs}"))
            case _                                                    => IO.unit
          }
          .map(job => BQSchema.fromSchema(job.getStatistics[QueryStatistics]().getSchema))
          .map(checkType.checkSchema)
      ...
      this match {
        case CheckType.Schema(expectedSchema) =>
          conforms(actualSchema, expectedSchema) match {
            case Some(reasons) => fail(s"Failed because ${reasons.mkString(", ")}")
  ...
  def dependenciesAsStaticData(frag: BQSqlFrag, assertStable: Seq[BQTableLike[Any]]): Either[String, BQSqlFrag] = {
    val (newFrag, ctes) = recurse(frag)
    val structured = BQStructuredSql.parse(newFrag)

    val unstableTables: Set[BQTableLike[Unit]] =
      newFrag.allReferencedAsPartitions.map(_.wholeTable.unpartitioned).toSet -- assertStable.map(_.unpartitioned)
    ...
    if (structured.queryType.toLowerCase != "select") {
      ...
      Left(s"References unstable tables ${unstableTables.map(t => formatTableId(t.tableId)).toList.sorted.mkString(", ")}")
    else
  ...
  private def recurse(frag: BQSqlFrag): (BQSqlFrag, List[CTE]) =
    frag match {
      ...
      case BQSqlFrag.Call(udf, args) =>
        ...
        (BQSqlFrag.Call(udf.copy(body = newUdfBody), newArgs), ctesFromUDF ++ ctes.flatten)
      ...
      case BQSqlFrag.Combined(frags) =>
        ...
        val newQuery =
          if (allDataRefs) newQueries.map(frag => bqfr"(select * from $frag)").mkFragment("(", " UNION ALL ", ")")
          else
      ...
      case p @ BQSqlFrag.PartitionRef(pid) =>
        ...
        schemaOpt match {
          case Some(schema) =>
            ...
            (cteName.bqShow, List(CTE(cteName, bqfr"(select as value ${exampleRow(schema)})")))
          case None =>
            (p, Nil)
        }
      ...
      case BQSqlFrag.FillRef(fill) =>
        ...
        (cteName.bqShow, List(CTE(cteName, bqfr"(select as value ${exampleRow(fill.tableDef.schema)})")))
    }
  ...
  def exampleRow(schema: BQSchema): BQSqlFrag = {
    ...
    def valueForType(field: BQField): BQSqlFrag = {
      val base = field.tpe match {
        ...
        case StandardSQLTypeName.BYTES      => BQSqlFrag("(select HLL_COUNT.INIT(x) from unnest(['a']) x)").bqShow // how to write a literal?
        case StandardSQLTypeName.STRUCT     => struct(field.subFields)
        case StandardSQLTypeName.ARRAY      => List.range(0, 2).map(_ => valueForType(field.copy(tpe = field.subFields.head.tpe))).mkFragment("[", ", ", "]")
        case StandardSQLTypeName.TIMESTAMP  => BQSqlFrag("TIMESTAMP('2020-01-01 00:00:00+00')")
        case StandardSQLTypeName.DATE       => BQSqlFrag("DATE(2020, 1, 1)")
        case StandardSQLTypeName.TIME       => BQSqlFrag("TIME(12, 0, 0)")
        case StandardSQLTypeName.DATETIME   => BQSqlFrag("DATETIME(2020, 1, 1, 00, 00, 00)")
        case StandardSQLTypeName.GEOGRAPHY  => BQSqlFrag("ST_GeogFromText('POINT(0 0)')")
        case StandardSQLTypeName.INTERVAL   => BQSqlFrag("MAKE_INTERVAL(1, 6, 15)")
      }
  ...
  case class CachedQuery(frag: BQSqlFrag) {
    val cacheFile = Paths.basedir.resolve("smoke-test-cache").resolve(s"${frag.asStringWithUDFs.hashCode()}.json")
