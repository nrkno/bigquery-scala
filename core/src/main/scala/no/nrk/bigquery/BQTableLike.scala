...
object BQTableLike {
  implicit def encodes[P]: Encoder[BQTableLike[P]] = Encoder.instance[BQTableLike[P]] {
    case x: BQTableRef[_] =>
      Json.obj(
        "tableId" := x.tableId,
  ...
  implicit class ExtensionMethods(private val table: BQTableLike[Any]) extends AnyVal {
    def loadGenericPartitions(
      ...
      PartitionLoader.loadGenericPartitions(table, client, startDate, requireRowNums)
  }

  implicit class ConditionallyAddedExtensionMethods[P](private val table: BQTableLike[P]) extends AnyVal {
    def assertPartition(partition: P)(implicit P: TableOps[P]): BQPartitionId[P] =
      P.assertPartition(table, partition)

    def loadPartitions(client: BigQueryClient, startDate: StartDate[P], requireRowNums: Boolean = false)(implicit
        P: TableOps[P]
    ): IO[Vector[(BQPartitionId[P], PartitionMetadata)]] =
      P.loadPartitions(table, client, startDate, requireRowNums)
  }

  implicit class ConditionallyAddedExtensionMethodsUnit(private val table: BQTableLike[Unit]) extends AnyVal {
    def assertPartition: BQPartitionId[Unit] =
      TableOps.unit.assertPartition(table, ())

    def loadPartition(client: BigQueryClient): IO[(BQPartitionId[Unit], PartitionMetadata)] =
...
/** A reference to a table we know is partitioned in one of several ways.
  ...
  *
  * 1) Look up all partitions through `allPartitions`. This works with any `BQPartitionedTable`. 2) Construct a legal [[BQPartitionId]] for it with
  * `assertPartition`.
  *
  * The design of this tri-structure was a bit problematic, especially regarding variance.
  *
  * This iteration of the design uses covariant type parameters throughout, with the caveat that when you combine (say) a non-partitioned and a date-partitioned
  * table into a list (or otherwise lose precise types), you also lose the ability to construct legal [[BQPartitionId]] s with `assertPartition`
  */
case class BQTableRef[+P](tableId: TableId, partitionType: BQPartitionType[P]) extends BQTableLike[P] {
...
object BQTableDef {
  implicit def encodes[P]: Encoder[BQTableDef[P]] = Encoder.instance[BQTableDef[P]] {
  ...
  case class Table[+P](
    ...
    override def withTableType[NewParam](tpe: BQPartitionType[NewParam]): Table[NewParam] =
  ...
  case class View[+P](
    ...
    override def withTableType[NewParam](tpe: BQPartitionType[NewParam]): View[NewParam] =
  ...
  /** Note that none of these fields can be changed after the MV has been created.
    ...
    *
    * @param schema
    *   *NOT* used for creating the MV because it's not possible yet. However, we use it to test that `query` conforms, we can use it for docs and so on.
    */
  case class MaterializedView[+P](
    ...
    override def withTableType[PP](tpe: BQPartitionType[PP]): MaterializedView[PP] =
      MaterializedView(tableId, tpe, query, schema, enableRefresh, refreshIntervalMs, description, labels)
  }
}
