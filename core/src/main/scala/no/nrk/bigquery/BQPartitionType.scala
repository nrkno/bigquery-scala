...
/** Describes a way a BQ table can be partitioned (or not).
  *
  * As as user you're expected to use this type to "tag" tables, but it doesn't
  * expose much functionality directly. Instead, it binds together several type
  * and value mappings so that the rest of the machinery works.
  *
  * It exists as its own type because it's shared between [[BQTableRef]] and
  * [[BQTableDef.Table]].
  *
...
object BQPartitionType {
  implicit def encodes[Param]: Encoder[BQPartitionType[Param]] = pt =>
    Json.fromString(pt.toString)

  final case class DatePartitioned(field: Ident)
      extends BQPartitionType[LocalDate] {
  ...
  final case class MonthPartitioned(field: Ident)
      extends BQPartitionType[YearMonth] {
    override def timePartitioning: Option[TimePartitioning] =
      Some(
        TimePartitioning
  ...
  case class IgnoredPartitioning[P](original: BQPartitionType[P])
      extends NotPartitioned
  ...
  def from(
    ...
    (timePartitioning, rangePartitioning) match {
      ...
      case (Some(time), None)
          if time.getType == TimePartitioning.Type.DAY && time.getField != null =>
        Right(BQPartitionType.DatePartitioned(Ident(time.getField)))
      case (Some(time), None)
          if time.getType == TimePartitioning.Type.MONTH && time.getField != null =>
