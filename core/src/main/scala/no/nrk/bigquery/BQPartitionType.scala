...
import com.google.cloud.bigquery.{MaterializedViewDefinition, RangePartitioning, StandardTableDefinition, TimePartitioning}
...
/** Describes a way a BQ table can be partitioned (or not).
  *
  * As as user you're expected to use this type to "tag" tables, but it doesn't expose much functionality directly. Instead, it binds together several type and
  * value mappings so that the rest of the machinery works.
  *
  ...
  *
  * As such the [[BQPartitionType]] establishes a mapping from [[Param]] to [[Id]].
  *
  * It is also a mapping back and forth to [[TimePartitioning]] / [[RangePartitioning]]
  *
...
object BQPartitionType {
  ...
  final case class DatePartitioned(field: Ident) extends BQPartitionType[LocalDate] {
    override def timePartitioning: Option[TimePartitioning] =
      Some(TimePartitioning.newBuilder(TimePartitioning.Type.DAY).setField(field.value).build())
  ...
  final case class MonthPartitioned(field: Ident) extends BQPartitionType[YearMonth] {
    override def timePartitioning: Option[TimePartitioning] =
      Some(TimePartitioning.newBuilder(TimePartitioning.Type.MONTH).setField(field.value).build())
  ...
  def from(actual: StandardTableDefinition): Either[String, BQPartitionType[Any]] =
    from(Option(actual.getTimePartitioning), Option(actual.getRangePartitioning))

  def from(actual: MaterializedViewDefinition): Either[String, BQPartitionType[Any]] =
    from(Option(actual.getTimePartitioning), Option(actual.getRangePartitioning))

  def from(timePartitioning: Option[TimePartitioning], rangePartitioning: Option[RangePartitioning]): Either[String, BQPartitionType[Any]] =
    (timePartitioning, rangePartitioning) match {
      ...
      case (time, range) =>
        Left(s"Need to implement support in `BQPartitionType` for ${time.orElse(range)}")
    }
}
