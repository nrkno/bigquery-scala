...
object BQRead {
  ...
  private def firstNotNullable(schema: avro.Schema): Option[avro.Schema] =
    if (schema.isUnion) schema.getTypes.asScala.collectFirst { case s if !s.isNullable => s }
    else None
    ...
    new BQRead[T] {
      override val bqType: BQType =
        BQType(
          ...
          ctx.parameters.map(param => param.label -> param.typeclass.bqType).toList
        )
  ...
  implicit def convertsOption[A: BQRead]: BQRead[Option[A]] =
    new BQRead[Option[A]] {
      override val bqType: BQType = {
        ...
        if (base.mode == Field.Mode.REQUIRED) base.copy(mode = Field.Mode.NULLABLE)
        else base
      }

      override def read(transportSchema: avro.Schema, value: Any): Option[A] =
        if (value == null) None
        else Some(BQRead[A].read(firstNotNullable(transportSchema).getOrElse(transportSchema), value))
    }

  implicit def convertsIterable[I[t] <: Iterable[t], A: BQRead](implicit cb: Factory[A, I[A]]): BQRead[I[A]] =
    new BQRead[I[A]] {
      override val bqType: BQType =
  ...
  implicit val convertsString: BQRead[String] =
    new BQRead[String] {
      ...
      override def read(transportSchema: avro.Schema, value: Any): String = value match {
        case str: String => str
  ...
  implicit val convertsDouble: BQRead[Double] =
    new BQRead[Double] {
      ...
      override def read(transportSchema: avro.Schema, value: Any): Double = value match {
  ...
  implicit val convertsBoolean: BQRead[Boolean] =
    new BQRead[Boolean] {
      ...
      override def read(transportSchema: avro.Schema, value: Any): Boolean = value match {
