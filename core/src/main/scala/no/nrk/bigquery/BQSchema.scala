...
case class BQSchema(fields: List[BQField]) {
  ...
  /** This is useful for views, because we're unable to create views with required columns.
    *
    * This is enforced by BQ at create-time because we *have* to create the view without a schema, and then we may optionally update the view to add our schema,
    * and in that case it can only legally extend what was inferred.
    *
    * @return
...
object BQSchema {
  ...
  implicit val encodes: Encoder[BQSchema] = Encoder.forProduct1("fields")(_.fields)
  ...
  def fromSchema(schema: Schema): BQSchema =
    schema match {
      ...
      case schema => BQSchema(schema.getFields.asScala.toList.map(BQField.fromField))
    }
}
