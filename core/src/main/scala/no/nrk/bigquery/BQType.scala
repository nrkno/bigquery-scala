...
/** This is the schema of a BQ type, where fields don't have names until they are placed inside a struct.
  *
  * We also use it for query responses, where single-column corresponds to a non-struct `BQType`, and otherwise all chosen columns are packed into a struct
  */
case class BQType(mode: Field.Mode, tpe: StandardSQLTypeName, subFields: List[(String, BQType)]) {
  ...
  def asSchema: Either[String, BQSchema] = {
    def go(name: String, tpe: BQType): BQField =
      BQField(name, tpe.tpe, tpe.mode, subFields = tpe.subFields.map { case (name, tpe) => go(name, tpe) })
...
object BQType {
  ...
  val FLOAT64: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.FLOAT64, Nil)
  val NUMERIC: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.NUMERIC, Nil)
  val BIGNUMERIC: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.BIGNUMERIC, Nil)
  val STRING: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.STRING, Nil)
  val BYTES: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.BYTES, Nil)
  val TIMESTAMP: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.TIMESTAMP, Nil)
  ...
  val DATETIME: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.DATETIME, Nil)
  val GEOGRAPHY: BQType = apply(Field.Mode.REQUIRED, StandardSQLTypeName.GEOGRAPHY, Nil)
  ...
  def fromField(field: BQField): BQType =
    BQType(field.mode, field.tpe, field.subFields.map(f => f.name -> fromField(f)))

  def fromBQSchema(schema: BQSchema): BQType =
    schema.fields match {
      case List(one) => fromField(one)
      case more      => BQType(Field.Mode.REQUIRED, StandardSQLTypeName.STRUCT, more.map(f => f.name -> fromField(f)))
    }

  def format(f: BQType): String =
    f match {
      case f if f.tpe == StandardSQLTypeName.STRUCT && f.mode == Field.Mode.REPEATED =>
        s"ARRAY<STRUCT<${f.subFields.map { case (name, sf) => s"$name ${format(sf)}" }.mkString(", ")}>>"
